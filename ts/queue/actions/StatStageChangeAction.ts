...
class StatStageChangeAction extends BattleAction {
	...
	async execute() {
		...
		if (this.amount > 0 && statAmountBeforeChange >= 6) infoText = `${this.target.displayName}'s ${Stats.toDisplayName(this.stat)} won't go any higher!`;
		if (this.amount < 0 && statAmountBeforeChange <= -6) infoText = `${this.target.displayName}'s ${Stats.toDisplayName(this.stat)} won't go any lower!`;

		if (infoText) rendererPromises.push(this.queue?.battle.renderer.showTextWhilePausingQueue(infoText));
